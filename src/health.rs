    ...
    async fn check_all_backends(&self) -> Result<()> {
        ...
        for backend in pool.all_backends() {
            ...
            let handle = tokio::spawn(async move {
                let addr = match backend.config.to_health_check_addr() {
                    Ok(a) => a,
                    Err(e) => {
                ...
                debug!("Health check: {}:{}", backend.config.host, backend.config.port);
                ...
                match result {
                    ...
                    Ok(Err(e)) => {
                        // Connection failure
                        debug!("Backend {}:{} connection failed: {}", backend.config.host, backend.config.port, e);
                        backend.mark_failure(HEALTH_CHECK_MAX_RETRIES);
                    }
                    Err(_) => {
                        // Timeout
                        debug!("Backend {}:{} timeout", backend.config.host, backend.config.port);
                        backend.mark_failure(HEALTH_CHECK_MAX_RETRIES);
                    }
                }
            });
