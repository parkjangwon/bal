...
impl ConfigStore {
    ...
    pub async fn validate_and_load(path: &Path) -> Result<RuntimeConfig> {
        ...
        for backend in &config.backends {
            match backend.check_connectivity().await {
                Ok(()) => {
                    debug!("  [OK] {}:{} - Connection successful", backend.host, backend.port);
                }
                Err(e) => {
        ...
        if failed_count > 0 {
            warn!("{} backends are unreachable. Some traffic may fail.", failed_count);
        }
    ...
    pub async fn load_initial_config(cli_path: Option<&Path>) -> Result<(RuntimeConfig, std::path::PathBuf)> {
        let path = if let Some(p) = cli_path {
            // Use path specified via CLI
            if !p.exists() {
                bail!("Specified configuration file does not exist: {}", p.display());
            }
            p.to_path_buf()
        } else {
